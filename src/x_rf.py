    ...
    for e in n_estimators:
        for s in min_samples_split:
            for d in max_depth:
                ...
                key = "n_estimators={}, min_samples_split={}, max_depth={}".format(
                    str(e), str(s), str(d)
                )
                models[key] = model
...
def main():
    ...
    argParser.add_argument(
        "--n-estimators", nargs="+", type=int, default=[200], help="Num RF estimators"
    )
    argParser.add_argument(
        "--min-samples-split",
        nargs="+",
    ...
    argParser.add_argument(
        "--max-depth", nargs="+", type=int, default=[9], help="Max RF depth"
    )
    argParser.add_argument(
        "--random-state", type=int, default=8, help="RF random state"
    )
    argParser.add_argument(
        "-t",
        "--task",
