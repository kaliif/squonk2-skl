...
def run(
    ...
    X, y, _ = utils.read_csv(
        infile, read_header, delimiter, id_column, mol_column, y_column
    )

    utils.run_grid_search(
        RandomForestRegressor(random_state=random_state), param_grid, scorers, X, y
    )
...
def main():
    ...
    parser.add_argument(
        "--n-estimators", nargs="+", type=int, default=[100], help="Num RF estimators"
    )
    parser.add_argument(
        "--min-samples-split",
        nargs="+",
        type=int,
        default=[2],
        help="Min RF samples split",
    )
    parser.add_argument(
        "--max-depth", nargs="+", type=int, default=[None], help="Max RF depth"
    )
