...
def run(
    ...
    df = pd.read_csv(
        infile, sep=delimiter, header=header, index_col=False, low_memory=False
    )
    ...
    if remove_non_numeric:
        ...
        for k in X.keys():
            if not is_numeric_dtype(X[k]):
                ...
                del X[k]
        DmLog.emit_event(
            "removed {} columns. shape is now {}".format(removed_count, X.shape)
        )
    ...
    if outfile:
        with open(outfile, "wt", encoding="utf-8") as out:
            ...
            DmLog.emit_event("written output to", outfile)
...
def main():
    ...
    parser.add_argument("-o", "--outfile", help="Output file (.smi)")

    parser.add_argument(
        "-t", "--variance-threshold", type=float, help="Variance threshold"
    )
    parser.add_argument(
        "--remove-non-numeric",
        action="store_true",
    ...
    parser.add_argument(
        "--mol-column", type=int, help="Column index for molecule when using .smi"
    )
    parser.add_argument(
        "--y-column", type=int, help="Column index for the Y variable when using .smi"
    )
    parser.add_argument(
        "--read-header",
        action="store_true",
    ...
    run(
        ...
        remove_non_numeric=args.remove_non_numeric,
