...
def run(inputfile, delimiter=',', read_header=True, id_column=0, mol_column=1, y_column=2):

    utils.validate_args(id_column, mol_column, y_column)

    header = 0 if read_header else None

    df = pd.read_csv(inputfile, sep=delimiter, header=header, index_col=None, low_memory=False)

    PandasTools.AddMoleculeColumnToFrame(df, smilesCol=df.columns[mol_column])

    print(df.shape)

    mol_list_train, mol_list_test, y_train, y_test = train_test_split(df.ROMol, df[df.columns[y_column]], random_state=0)
